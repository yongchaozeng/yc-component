<template>
    ...
    <ul :class="wrapClasses" :style="styles" v-else>
        <span :class="[prefixCls + '-total']" v-if="showTotal">
            <slot>{{ '总数量' }} {{ total }} <template v-if="total <= 1">{{ '名称' }}</template><template
                    v-else>{{ '名称' }}</template></slot>
        </span>
        <li
                :title="'名称'"
                :class="prevClasses"
        ...
        <li :title="'名称'" v-if="currentPage > 5" :class="[prefixCls + '-item-jump-prev']" @click="fastPrev"><a><i
        ...
        <li :title="currentPage + 3" v-if="allPages - currentPage === 4" :class="[prefixCls + '-item']"
            @click="changePage(currentPage + 3)"><a>{{ currentPage + 3 }}</a></li>
        <li :title="'名称'" v-if="allPages - currentPage >= 5" :class="[prefixCls + '-item-jump-next']" @click="fastNext">
            <a><i class="ivu-icon ivu-icon-ios-arrow-forward">...</i></a></li>
...
<script>
  /* iview的分页 */
  import Options from './pc-page-option.vue'
